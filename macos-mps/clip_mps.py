# Modified for MacOS MPS compatibility
# Removed flash_attn dependency, using PyTorch's scaled_dot_product_attention
...
class NoTPAttention(torch.nn.Module):
    """MPS-compatible attention module using PyTorch's scaled_dot_product_attention."""
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Always use PyTorch's scaled_dot_product_attention (MPS compatible)
...
if __name__ == '__main__':
    from sam_vary_mps import build_sam_vit_b
    ...
    with torch.no_grad():
        ...
        print(f"SAM output shape: {patch_embed.shape}")
        y = vision_model(x, patch_embed)
        print(f"CLIP output shape: {y.shape}")

        image_feature = torch.add(y[:, 1:], patch_embed.flatten(2).permute(0, 2, 1))
        print(f"Combined feature shape: {image_feature.shape}")
